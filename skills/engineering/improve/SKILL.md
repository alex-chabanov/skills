...
- Read `README`, `CLAUDE.md`/`AGENTS.md`, `CONTRIBUTING`, root config files (`package.json`, `pyproject.toml`, `go.mod`, `build.gradle`/`build.gradle.kts` + `settings.gradle*` + `gradle/libs.versions.toml`, `Package.swift`, `*.xcodeproj`/`*.xcworkspace` + `Podfile`, etc.), CI config, and the directory structure.
- Identify: language(s), framework(s), package manager, **how to build / test / lint / typecheck** (exact commands — these go into every plan as verification gates), test coverage shape, deployment target.
- **Detect the platform and carry it forward.** A mobile/native target changes which risks dominate and which commands verify them. Signals: Android/Kotlin (`AndroidManifest.xml`, `*.gradle*`, `gradle/libs.versions.toml`, `gradlew`) → build/test/lint via `./gradlew assembleDebug` / `testDebugUnitTest` / `lint` / `ktlintCheck` / `detekt`; Apple/Swift (`Package.swift`, `*.xcodeproj`/`*.xcworkspace`, `Info.plist`) → `swift build` / `xcodebuild test` / `swift test`. When a mobile platform is detected, tell each Audit subagent to weight the **Mobile lens** bullets in its playbook section, and prefer a scoped verify (single-module `testDebugUnitTest`, one scheme's `xcodebuild test`) over a full build in every plan — native builds are slow and the executor loops on them.
